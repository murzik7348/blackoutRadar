...
def _load_zakarpattia() -> dict:
    ...
    path = os.path.join(os.path.dirname(__file__), "backend", "zakarpattia_settlements.json")
...
def _queue_main_keyboard() -> InlineKeyboardMarkup:
    ...
    # Кнопка зміни локації, як у тебе було
...
async def restart_regions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Спробуємо відправити повідомлення заново
        try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Відправляємо клавіатуру користувачу. Якщо update має message — відповідаємо на нього,
    # інакше використовуємо effective_chat як запасний варіант.
    if update.message:
        ...
        # іноді старт може бути викликаний в контексті, де message відсутній
...
async def on_region_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if region != "Закарпатська область":
        # Показуємо повідомлення, що ця область ще не підтримується
        message_text = (
...
async def on_type_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Закарпатська область вже перевірена в on_region_chosen, тому тут завжди Закарпатська
...
async def handle_manual_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Перевіряємо, чи не порожнім
    if not name:
        await update.message.reply_text(
            "❌ Будь ласка, введіть назву населеного пункту (не можна залишати порожнім)"
        )
        return ENTER_MANUAL
...
async def on_queue_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = q.data.split("|")
    if len(parts) != 2:
        logger.error(f"❌ Невалідні дані queue_sub: {q.data}")
        await q.edit_message_text("❌ Помилка обробки вибору черги. Спробуйте знову.")
        return ASK_QUEUE_SUB
    
    _, full_q = parts
    queue = full_q.strip()
    
    # Перевіряємо формат черги (має бути "N.N")
    if not queue or not all(part.isdigit() for part in queue.split(".")):
        logger.warning(f"⚠️ Невалідний формат черги: {queue}")
        await q.edit_message_text("❌ Невалідна черга. Спробуйте знову.")
        return ASK_QUEUE_SUB
    ...
    return ConversationHandler(
        ...
        states={
            SELECT_REGION: [
                ...
                CallbackQueryHandler(restart_regions, pattern=r"^restart_regions$"),  # Додано для обробки кнопки повернення
            ],
            CHOOSE_TYPE: [
            ...
            ENTER_MANUAL: [
                ...
                CallbackQueryHandler(restart_regions, pattern=r"^restart_regions$"),  # Додано для можливості повернутися
            ],
            ASK_QUEUE_MAIN: [
...
def get_handlers():
    return [get_conversation_handler()]
